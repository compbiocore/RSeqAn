...
```{r, vs_shortreads, include=FALSE, warning=FALSE}
...
qckit1<-benchmark(adapter_content(fls,nr=20000))
...
qckit2<-benchmark(adapter_content(infile,nr=14))
...
```{r, table, echo=FALSE}
kable(table,caption="Benchmark results of qckitfastq, which uses RSeqAn, and ShortRead, which does not against a test file from each package.") %>%
